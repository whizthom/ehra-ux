...
import { createPublicOrder, getPublicProducts, getPublicStorefront, getCustomerOverview } from "../../api/commerceApi";
import { buildWhatsAppLink } from "../../api/whatsappApi";
import { verifyOtp, checkPhone, checkPhoneBeforeOtp, sendCustomerRegistrationOtp, registerCustomerWithPhone } from "../../api/phoneAuthApi";
...
const getCategory = (p) => {
  const raw = p?.category ?? p?.productCategory ?? p?.categoryName ?? (typeof p?.category === "object" ? p.category?.name : null);
...
const stockState = (p) => {
  ...
  if (n !== null) {
    ...
    if (Number.isFinite(threshold) && threshold > 0 && n <= threshold) return { label: `${n} left in stock`, tone: "low", quantity: n };
    return { label: `${n} in stock`, tone: "good", quantity: n };
  }
  return isInStock(p) ? { label: "Available", tone: "good", quantity: null } : { label: "Out of stock", tone: "empty", quantity: 0 };
};
const imagesOf = (p) => {
  try {
...
function ProductCard({ product, onOpen, onAdd, wished, onWishlist, compact = false }) {
  const image = imagesOf(product)[0]; const discounted = Number(product?.discount || 0) > 0; const stock = stockState(product);
  return <article className={`${styles.product} ${compact ? styles.productCompact : ""}`}>
    <button type="button" className={styles.productVisual} onClick={() => onOpen(product)} aria-label={`View ${product.name}`}>
      {image ? <img src={image} alt={product.name} loading="lazy" /> : <div className={styles.imagePlaceholder}><i className="ti ti-package" /></div>}
      <span className={`${styles.stockBadge} ${styles[`stock${stock.tone}`]}`}>{stock.label}</span>{discounted && <span className={styles.saleBadge}>Offer</span>}
      <span className={styles.quickView}>Quick view <i className="ti ti-arrow-up-right" /></span>
    </button>
    <div className={styles.productBody}><div className={styles.productMeta}><span className={styles.categoryLabel}><i className="ti ti-tag" />{getCategory(product)}</span><span className={`${styles.cardStockText} ${styles[`stock${stock.tone}`]}`}>{stock.label}</span><button type="button" className={`${styles.heart} ${wished ? styles.wished : ""}`} onClick={() => onWishlist(product)}><i className={wished ? "ti ti-heart-filled" : "ti ti-heart"}/></button></div>
      <button type="button" className={styles.productName} onClick={() => onOpen(product)}>{product.name}</button>{!compact && product.description && <p className={styles.productDescription}>{product.description}</p>}
      <div className={styles.productFoot}><div><strong>{money(product.currency, effectivePrice(product))}</strong>{discounted && <del>{money(product.currency, product.price)}</del>}</div><button className={styles.addButton} disabled={!isInStock(product)} onClick={() => onAdd(product)}><i className="ti ti-plus"/></button></div>
    </div></article>;
}
...
function CustomerGate({ store, slug, gate, setGate, onComplete }) {
  ...
  const close = () => setGate((g) => ({ ...g, open: false, pending: null, message: "" }));
  const setMessage = (message) => setGate((g) => ({ ...g, message }));

  const submitPhone = async () => {
    const phone = gate.phone?.trim();
    if (!phone || !isValidPhoneNumber(phone)) { setMessage("Enter a valid phone number before continuing."); return; }
    try {
      ...
      setGate((g) => ({ ...g, step: "checking", message: "Checking your Ehral account…" }));
      ...
      if (check?.exists) {
        setGate((g) => ({
          ...
          message: "This phone number already has an Ehral account. Please sign in instead, or use Forgot password if you need to reset it."
        }));
        return;
      }
      setGate((g) => ({ ...g, step: "sending", message: "Sending verification code…" }));
      const r = await sendCustomerRegistrationOtp(phone);
      setGate((g) => ({ ...g, phone, step: "verify", pinId: r?.pinId, otp: "", message: r?.developmentOtp ? `Development OTP: ${r.developmentOtp}` : "Verification code sent to your phone." }));
    } catch (e) {
      if (e?.response?.status === 409) {
        setGate((g) => ({ ...g, phone, step: "existingLogin", password: "", message: e?.response?.data?.message || "This phone number already has an Ehral account. Please sign in or use Forgot password." }));
      } else {
        setGate((g) => ({ ...g, step: "phone", message: e?.response?.data?.message || "Could not check this phone number." }));
      }
    }
  };
  ...
  const verify = async () => {
    ...
    try {
      setGate((g) => ({ ...g, step: "verifying", message: "Verifying your phone number…" }));
      ...
      setGate((g) => ({ ...g, phone: v.phoneNumber || g.phone, phoneVerificationToken, step: check.exists ? "existingLogin" : "password", message: check.exists ? "This phone already has an Ehral account. Please sign in instead." : "Phone number verified. Create your password to continue." }));
    } catch (e) { setGate((g) => ({ ...g, step: "verify", message: e?.response?.data?.message || "The verification code could not be confirmed." })); }
  };
  ...
  const continueToProfile = () => {
    ...
    setGate((g) => ({ ...g, step: "profile", message: "Password created. Now complete your basic profile." }));
  };
  ...
  const createAccount = async () => {
    ...
    try {
      setGate((g) => ({ ...g, step: "creating", message: "Creating your secure Ehral customer account…" }));
      await registerCustomerWithPhone(gate.phoneVerificationToken, { businessSlug: slug, firstName: gate.firstName.trim(), lastName: gate.lastName.trim(), email: gate.email.trim(), password: gate.password });
      clearTokens();
      navigate("/login", { state: { phone: gate.phone, message: "Your Ehral customer account is ready. Sign in with your password to continue." } });
      setGate((g) => ({ ...g, open: false, pending: null, step: "done", message: "" }));
    } catch (e) { setGate((g) => ({ ...g, step: "profile", message: e?.response?.data?.message || "Could not create your customer account." })); }
  };

  const signInExisting = async () => {
    if (!gate.password) { setMessage("Enter your Ehral account password."); return; }
    try {
      setGate((g) => ({ ...g, step: "signingIn", message: "Signing you into Ehral…" }));
      await apiLogin(gate.phone, gate.password);
      const accounts = await getMyAccounts();
      const list = Array.isArray(accounts) ? accounts : [];
      const existing = list.find((a) => a?.type === "CUSTOMER" && String(a?.businessId) === String(store?.businessId));
      if (existing?.membershipId) await switchContext("CUSTOMER", existing.membershipId);
      else {
        await connectCustomerToBusiness(slug);
        const refreshed = await getMyAccounts();
        const membership = (Array.isArray(refreshed) ? refreshed : []).find((a) => a?.type === "CUSTOMER" && String(a?.businessId) === String(store?.businessId));
        if (membership?.membershipId) await switchContext("CUSTOMER", membership.membershipId);
      }
      let profile = {};
      try { const profileResponse = await getCustomerOverview(); profile = profileResponse?.data || {}; } catch {}
      onComplete({ name: [profile.firstName || gate.firstName, profile.lastName].filter(Boolean).join(" "), phone: profile.phone || gate.phone, email: profile.email || gate.email, action: gate.pending });
    } catch (e) { setGate((g) => ({ ...g, step: "existingLogin", message: e?.response?.data?.message || "We could not sign you in. Check your password and try again." })); }
  };

  if (!gate.open) return null;
  const stepNumber = ["phone", "checking", "sending"].includes(gate.step) ? 1 : ["verify", "verifying"].includes(gate.step) ? 2 : 3;
  ...
  return (
    <div className={styles.overlay} role="dialog" aria-modal="true" aria-label="Ehral customer account">
      <div className={`${styles.accountModal} ${styles.customerGateModal}`}>
        ...
        <div className={styles.customerGateBrand}>
          <div className={styles.customerGateEhral}><Logo size={112} variant="horizontal" title="Ehral" tone="brand" /><span className={styles.customerGatePowered}>EHRAL CUSTOMER</span></div>
          {store?.businessLogo && <><span className={styles.customerGateBrandDivider} aria-hidden="true" /><span className={styles.customerGateStoreLogo}><img src={store.businessLogo} alt="" /></span></>}
        </div>
        <div className={styles.customerGateBrandRule} aria-hidden="true"><span /><i /><span /></div>
        <div className={styles.customerGateProgress} aria-label={`Step ${stepNumber} of 3`}>{[1,2,3].map((n)=><span key={n} className={n<=stepNumber?styles.customerGateProgressActive:""}>{n}</span>)}</div>
        <span className={styles.sectionKicker}>{isLogin ? "WELCOME BACK" : "EHRAL CUSTOMER ACCOUNT"}</span>
        <h2>{stepNumber === 1 ? "Start with your phone number" : stepNumber === 2 ? "Verify your phone number" : isLogin ? "Sign in to continue" : isPasswordStep ? "Create your password" : "Complete your profile"}</h2>
        <p>{stepNumber===1&&<>Use your phone number to securely connect with <b>{storeName}</b>.</>}{stepNumber===2&&<>We sent a verification code to <b>{gate.phone}</b>. Enter it below to continue.</>}{stepNumber===3&&!isLogin&&isPasswordStep&&<>Your phone is verified. Create the password you will use to sign in to your Ehral account.</>}{stepNumber===3&&!isLogin&&isProfileStep&&<>Password set. Add your basic details now. You can complete and edit your full profile later from your customer dashboard.</>}{isLogin&&<>An Ehral account already exists for <b>{gate.phone}</b>. Please sign in instead. If you do not remember your password, use the password reset link below.</>}</p>
        {gate.step === "phone" && <div className={styles.accountForm}><label className={styles.customerGateLabel}>Phone number</label><div className={styles.customerPhoneWrap}><PhoneInput international defaultCountry="NG" countryCallingCodeEditable={false} placeholder="Enter your phone number" value={gate.phone} onChange={(value)=>setGate(g=>({...g,phone:value||"",message:""}))} onKeyDown={(e)=>e.key==="Enter"&&submitPhone()} className={styles.customerPhoneInput}/></div><small className={styles.customerGateHint}>We verify your phone before requesting your personal details.</small><button disabled={!gate.phone||!isValidPhoneNumber(gate.phone)} onClick={submitPhone}>Continue securely <i className="ti ti-arrow-right"/></button></div>}
        {gate.step === "checking" && <div className={styles.gateLoading}><span className={styles.gateSpinner}/> Checking your Ehral account…</div>}
        {gate.step === "sending" && <div className={styles.gateLoading}><span className={styles.gateSpinner}/> Sending your verification code…</div>}
        {(gate.step === "verify"||gate.step === "verifying") && <div className={styles.accountForm}><label className={styles.customerGateLabel}>Verification code</label><input className={styles.otpInput} inputMode="numeric" autoComplete="one-time-code" autoFocus placeholder="Enter verification code" value={gate.otp} onChange={(e)=>setGate(g=>({...g,otp:e.target.value.replace(/\D/g,"").slice(0,8),message:""}))} onKeyDown={(e)=>e.key==="Enter"&&verify()}/><button disabled={!gate.otp||gate.step==="verifying"} onClick={verify}>{gate.step==="verifying"?"Verifying…":"Verify phone number"} <i className="ti ti-check"/></button><button type="button" className={styles.customerBackButton} onClick={()=>setGate(g=>({...g,step:"phone",otp:"",pinId:"",message:""}))}>Change phone number</button></div>}
        {gate.step === "password" && <div className={styles.accountForm}><div className={styles.verifiedPhone}><i className="ti ti-circle-check-filled"/><span>{gate.phone}</span><b>Phone verified</b></div><div className={styles.passwordPair}><div><label className={styles.customerGateLabel}>Create password</label><input autoFocus type="password" autoComplete="new-password" placeholder="At least 8 characters" value={gate.password} onChange={(e)=>setGate(g=>({...g,password:e.target.value,message:""}))}/></div><div><label className={styles.customerGateLabel}>Confirm password</label><input type="password" autoComplete="new-password" placeholder="Repeat password" value={gate.confirmPassword} onChange={(e)=>setGate(g=>({...g,confirmPassword:e.target.value,message:""}))}/></div></div><small className={styles.customerGateHint}>This password will be used to sign in to your Ehral account. Your full profile can be completed later from the dashboard.</small><button disabled={gate.password.length<8||gate.password!==gate.confirmPassword} onClick={continueToProfile}>Continue to profile <i className="ti ti-arrow-right"/></button></div>}
        {(gate.step === "profile") && <div className={styles.accountForm}><div className={styles.verifiedPhone}><i className="ti ti-circle-check-filled"/><span>{gate.phone}</span><b>Verified</b></div><div className={styles.twoCol}><div><label className={styles.customerGateLabel}>First name</label><input autoFocus required placeholder="First name" value={gate.firstName} onChange={(e)=>setGate(g=>({...g,firstName:e.target.value,message:""}))}/></div><div><label className={styles.customerGateLabel}>Last name</label><input placeholder="Last name" value={gate.lastName} onChange={(e)=>setGate(g=>({...g,lastName:e.target.value,message:""}))}/></div></div><div><label className={styles.customerGateLabel}>Email address <span>(optional)</span></label><input type="email" placeholder="you@example.com" value={gate.email} onChange={(e)=>setGate(g=>({...g,email:e.target.value,message:""}))}/></div><small className={styles.customerGateHint}>These are your basic registration details. You can add and edit your full profile from the customer dashboard after signing in.</small><button disabled={!gate.firstName.trim()||gate.step==="creating"} onClick={createAccount}>{gate.step==="creating"?"Creating account…":"Create my Ehral account"} <i className="ti ti-arrow-right"/></button></div>}
        {gate.step === "creating" && <div className={styles.gateLoading}><span className={styles.gateSpinner}/> Creating your secure Ehral account…</div>}
        {(gate.step === "existingLogin"||gate.step === "signingIn") && <div className={styles.accountForm}><div className={styles.verifiedPhone}><i className="ti ti-user-check"/><span>{gate.phone}</span><b>Ehral account found</b></div><label className={styles.customerGateLabel}>Password</label><input type="password" autoComplete="current-password" autoFocus placeholder="Your Ehral password" value={gate.password} onChange={(e)=>setGate(g=>({...g,password:e.target.value,message:""}))} onKeyDown={(e)=>e.key==="Enter"&&signInExisting()}/><button disabled={!gate.password||gate.step==="signingIn"} onClick={signInExisting}>{gate.step==="signingIn"?"Signing in…":"Sign in to Ehral"} <i className="ti ti-login-2"/></button><button type="button" className={styles.customerBackButton} onClick={()=>navigate("/forgot-password", { state: { phone: gate.phone } })}>Forgot password?</button><button type="button" className={styles.customerBackButton} onClick={()=>setGate(g=>({...g,step:"phone",password:"",message:""}))}>Use another phone number</button></div>}
        {gate.message && <small className={styles.formMessage} role="status">{gate.message}</small>}
        <small className={styles.privacyNote}><i className="ti ti-lock"/> Your Ehral account securely connects your shopping, orders, receipts and conversations across participating businesses.</small>
      </div>
    </div>
  );
}
...
export default function Storefront() {
  ...
  const categories = useMemo(() => ["All", ...Array.from(new Set(products.map(getCategory).filter(Boolean)))], [products]);
  const categoryGroups = useMemo(() => categories.slice(1).map((name) => ({ name, products: products.filter((p) => getCategory(p) === name) })).filter((g) => g.products.length), [categories, products]);
  const availableProducts = useMemo(() => products.filter(isInStock), [products]);
  ...
  const cartItems = useMemo(() => products.filter((p) => Number(cart[p.id]) > 0).map((p) => ({ ...p, quantity: Math.min(stockOf(p) !== null ? Math.floor(stockOf(p)) : 1000, Math.max(1, Number(cart[p.id]))), lineTotal: effectivePrice(p) * Number(cart[p.id]), lineTax: Number(p.tax || 0) * Number(cart[p.id]) })), [products, cart]);
  ...
  useEffect(() => {
    ...
    if (contextType !== "CUSTOMER" || !store?.businessId || String(businessId) !== String(store.businessId)) return;
    getCustomerOverview().then((r) => {
      const d = r?.data;
      const name = [d?.firstName, d?.lastName].filter(Boolean).join(" ").trim();
      setForm((f) => ({ ...f, customerName: name || f.customerName, customerPhone: d?.phone || f.customerPhone, customerEmail: d?.email || f.customerEmail }));
    }).catch(() => {});
  }, [store?.businessId]);
  ...
  const openProduct = useCallback((product) => { rememberProduct(product); setSelectedProduct(product); setProductQty(1); setProductImageIndex(0); setSearchOpen(false); }, [rememberProduct]);
  const add = useCallback((product) => setCart((c) => { const current = Number(c[product.id]) || 0; const stock = stockOf(product); const limit = stock !== null ? Math.floor(stock) : 1000; if (limit <= 0 || current >= limit) return c; return { ...c, [product.id]: Math.min(current + 1, limit) }; }), []);
  ...
  const requireCustomer = async (action) => {
    ...
    setCustomerGate((g) => ({ ...g, open: true, pending: action, step: "phone", pinId: "", otp: "", phoneVerificationToken: "", password: "", confirmPassword: "", message: "" }));
  };

  const chat = (product) => requireCustomer(async () => {
    try {
      const r = await createCustomerBusinessConversation(store.businessId);
      sessionStorage.setItem(`ehral:pending-chat:${r.data.id}`, `Hello, I am interested in ${product.name} (${money(product.currency, effectivePrice(product))}).`);
      navigate(`/customer-dashboard?chat=${r.data.id}`);
    } catch (e) { setActionError(e?.response?.data?.message || "Ehral messaging is temporarily unavailable."); }
  });
  const generalChat = () => requireCustomer(async () => {
    try {
      const r = await createCustomerBusinessConversation(store.businessId);
      sessionStorage.setItem(`ehral:pending-chat:${r.data.id}`, "Hello, I found your store on Ehral and would like to make an enquiry.");
      navigate(`/customer-dashboard?chat=${r.data.id}`);
    } catch (e) { setActionError(e?.response?.data?.message || "Ehral messaging is temporarily unavailable."); }
  });
  const openCheckout = () => {
  ...
  return (
    <div className={styles.page} style={{"--ehral-primary":"#0f6e56","--ehral-primary-dark":"#0b1f1a","--ehral-accent":"#55e0ae","--ehral-mint":"#eaf6f1","--ehral-line":"#b8ccc5"}}>
      ...
      <main id="top">
        ...
        <section className={styles.catalog} id="products">
          ...
          {filteredProducts.length ? (category === "All" && !search.trim() ? <div className={styles.categoryGroups}>{categoryGroups.map((group) => <section className={styles.categoryGroup} key={group.name}><div className={styles.categoryGroupHead}><div><span className={styles.categoryOverline}>COLLECTION</span><h3>{group.name}</h3></div><button type="button" onClick={() => setCategory(group.name)}>{group.products.length} {group.products.length === 1 ? "product" : "products"}<i className="ti ti-arrow-right" /></button></div><div className={styles.grid}>{group.products.map((p) => <ProductCard key={p.id} product={p} onOpen={openProduct} onAdd={(x) => requireCustomer(() => add(x))} wished={wishlist.has(String(p.id))} onWishlist={toggleWishlist} />)}</div></section>)}</div> : <div className={styles.grid}>{filteredProducts.map((p) => <ProductCard key={p.id} product={p} onOpen={openProduct} onAdd={(x) => requireCustomer(() => add(x))} wished={wishlist.has(String(p.id))} onWishlist={toggleWishlist} />)}</div>) : <div className={styles.empty}><i className="ti ti-search-off" /><h3>No products found</h3><p>Try a different search or category.</p><button onClick={() => { setSearch(""); setCategory("All"); setSort("featured"); }}>Clear filters</button></div>}
        </section>
      ...
      <footer className={styles.footer}><div className={styles.footerBrand}>{store.businessLogo ? <img src={store.businessLogo} alt="" /> : <span className={styles.logoFallback}>{String(store.name || "S").charAt(0).toUpperCase()}</span>}<div><strong>{store.name}</strong><span>Online storefront</span></div></div><div className={styles.footerLinks}><a href="#products">Shop</a>{store.whatsappNumber && <button onClick={generalChat}>Contact</button>}<span className={styles.ehralFooterBrand}><small>POWERED BY</small><Logo size={48} variant="horizontal" title="Ehral" tone="brand" /></span></div></footer>
      ...
      {selectedProduct && (() => { const gallery = imagesOf(selectedProduct); const stock = stockOf(selectedProduct); const state = stockState(selectedProduct); const maxQty = stock !== null ? Math.max(1, Math.floor(stock)) : (isInStock(selectedProduct) ? 1000 : 0); const meter = stock === 0 ? 4 : Math.min(100, Math.max(10, (stock / Math.max(stock, Number(selectedProduct.lowStockThreshold || 5) * 4)) * 100)); return <div className={styles.productOverlay} onMouseDown={(e) => { if (e.target === e.currentTarget) setSelectedProduct(null); }}><section className={styles.productSheet} role="dialog" aria-modal="true" aria-label={selectedProduct.name}><button className={styles.productClose} onClick={() => setSelectedProduct(null)} aria-label="Close"><i className="ti ti-x" /></button><div className={styles.productMedia}><div className={styles.mainProductImage}>{gallery.length ? <img src={gallery[Math.min(productImageIndex, gallery.length - 1)]} alt={selectedProduct.name} /> : <div className={styles.imagePlaceholder}><i className="ti ti-package" /></div>}<span className={`${styles.detailStockPill} ${styles[`stock${state.tone}`]}`}><i />{state.label}</span></div>{gallery.length > 1 && <div className={styles.thumbRail}>{gallery.map((u,i)=><button type="button" key={`${u}-${i}`} className={i===productImageIndex?styles.thumbActive:""} onClick={()=>setProductImageIndex(i)}><img src={u} alt="" /></button>)}</div>}</div><div className={styles.productDetail}><div className={styles.detailEyebrow}><span>{getCategory(selectedProduct)}</span>{selectedProduct.sku && <span>SKU {selectedProduct.sku}</span>}<button type="button" className={`${styles.detailHeart} ${wishlist.has(String(selectedProduct.id))?styles.wished:""}`} onClick={()=>toggleWishlist(selectedProduct)}><i className={wishlist.has(String(selectedProduct.id))?"ti ti-heart-filled":"ti ti-heart"}/></button></div><h2>{selectedProduct.name}</h2><div className={styles.detailPriceRow}><strong>{money(selectedProduct.currency, effectivePrice(selectedProduct))}</strong>{Number(selectedProduct.discount||0)>0&&<><del>{money(selectedProduct.currency, selectedProduct.price)}</del><span>Save {money(selectedProduct.currency, selectedProduct.discount)}</span></>}</div><p className={styles.detailDescription}>{selectedProduct.description || "A carefully selected product from this store."}</p><div className={styles.stockPanel}><div className={styles.stockPanelTop}><span>Current availability</span><strong>{stock > 0 ? `${stock} ${stock===1?"unit":"units"} in stock` : "Currently out of stock"}</strong></div><div className={styles.stockMeter}><span style={{width:`${meter}%`}}/></div>{stock > 0 && stock <= Number(selectedProduct.lowStockThreshold||5) ? <small>Only {stock} {stock===1?"unit":"units"} remaining. Order soon while available.</small> : <small>{selectedProduct.trackInventory ? "Live inventory count from the store." : "Stock availability is supplied by the store."}</small>}</div><div className={styles.detailPerks}><span><i className="ti ti-shield-check"/> Secure order</span>{store.pickupEnabled&&<span><i className="ti ti-building-store"/> Pickup available</span>}{store.deliveryEnabled&&<span><i className="ti ti-truck-delivery"/> Delivery available</span>}</div><div className={styles.detailPurchase}><div className={styles.quantityControl}><button type="button" onClick={()=>setProductQty(q=>Math.max(1,q-1))} disabled={productQty<=1}>−</button><b>{productQty}</b><button type="button" onClick={()=>setProductQty(q=>Math.min(maxQty,q+1))} disabled={productQty>=maxQty}>+</button></div><button className={styles.primaryPurchase} disabled={!isInStock(selectedProduct) || maxQty === 0} onClick={()=>requireCustomer(()=>{for(let i=0;i<productQty;i++) add(selectedProduct);setSelectedProduct(null);setCartOpen(true);})}>Add {productQty>1?`${productQty} items`:"to bag"}<i className="ti ti-arrow-right"/></button></div>{store.whatsappNumber&&<button type="button" className={styles.questionCta} onClick={()=>{setSelectedProduct(null);chat(selectedProduct)}}><i className="ti ti-brand-whatsapp"/> Have a question about this product?</button>}</div></section></div>; })()}
