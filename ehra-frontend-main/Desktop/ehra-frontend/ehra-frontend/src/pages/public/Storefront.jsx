...
import { sendOtp, verifyOtp, checkPhone, registerCustomerWithPhone } from "../../api/phoneAuthApi";
...
function CustomerGate({ store, slug, gate, setGate, onComplete }) {
  ...
  const submitPhone = async () => {
    ...
    } catch (e) { setGate((g) => ({ ...g, step: "phone", message: e?.response?.data?.message || "Could not send verification code." })); }
  };
  ...
  const verify = async () => {
    ...
    try {
      ...
      setGate((g) => ({ ...g, phone: v.phoneNumber || g.phone, phoneVerificationToken, step: check.exists ? "existingLogin" : "details", message: check.exists ? "This phone already has an Ehral account. Sign in to continue." : "Phone number verified. Complete your profile to create your Ehral customer account." }));
    } catch (e) { setGate((g) => ({ ...g, step: "verify", message: e?.response?.data?.message || "The verification code could not be confirmed." })); }
  };

  const createAccount = async () => {
    if (!gate.phoneVerificationToken || !gate.firstName.trim() || gate.password.length < 8 || gate.password !== gate.confirmPassword) {
      setMessage(gate.password !== gate.confirmPassword ? "Your passwords do not match." : "Create a password with at least 8 characters."); return;
    }
    try {
    ...
    } catch (e) { setGate((g) => ({ ...g, step: "details", message: e?.response?.data?.message || "Could not create your customer account." })); }
  };
  ...
  const stepNumber = ["phone", "sending"].includes(gate.step) ? 1 : ["verify", "verifying"].includes(gate.step) ? 2 : 3;
  ...
  return (
    <div className={styles.overlay} role="dialog" aria-modal="true" aria-label="Ehral customer account">
      <div className={`${styles.accountModal} ${styles.customerGateModal}`}>
        ...
        <h2>{stepNumber === 1 ? "Start with your phone number" : stepNumber === 2 ? "Verify your phone number" : isLogin ? "Sign in to continue" : "Create your secure account"}</h2>
        <p>{stepNumber===1&&<>Use your phone number to securely connect with <b>{storeName}</b>.</>}{stepNumber===2&&<>We sent a verification code to <b>{gate.phone}</b>. Enter it below to continue.</>}{stepNumber===3&&!isLogin&&<>Your phone is verified. Create your Ehral profile and password for future access across Ehral.</>}{isLogin&&<>An Ehral account already exists for <b>{gate.phone}</b>. Sign in with your password to continue.</>}</p>
        ...
        {gate.step === "details" && <div className={styles.accountForm}><div className={styles.verifiedPhone}><i className="ti ti-circle-check-filled"/><span>{gate.phone}</span><b>Verified</b></div><div className={styles.twoCol}><div><label className={styles.customerGateLabel}>First name</label><input autoFocus required placeholder="First name" value={gate.firstName} onChange={(e)=>setGate(g=>({...g,firstName:e.target.value,message:""}))}/></div><div><label className={styles.customerGateLabel}>Last name</label><input placeholder="Last name" value={gate.lastName} onChange={(e)=>setGate(g=>({...g,lastName:e.target.value,message:""}))}/></div></div><div><label className={styles.customerGateLabel}>Email address <span>(optional)</span></label><input type="email" placeholder="you@example.com" value={gate.email} onChange={(e)=>setGate(g=>({...g,email:e.target.value,message:""}))}/></div><div className={styles.passwordPair}><div><label className={styles.customerGateLabel}>Create password</label><input type="password" autoComplete="new-password" placeholder="At least 8 characters" value={gate.password} onChange={(e)=>setGate(g=>({...g,password:e.target.value,message:""}))}/></div><div><label className={styles.customerGateLabel}>Confirm password</label><input type="password" autoComplete="new-password" placeholder="Repeat password" value={gate.confirmPassword} onChange={(e)=>setGate(g=>({...g,confirmPassword:e.target.value,message:""}))}/></div></div><small className={styles.customerGateHint}>Your Ehral password lets you return to your customer dashboard and access every business connected to your account.</small><button disabled={!gate.firstName.trim()||gate.password.length<8||gate.password!==gate.confirmPassword} onClick={createAccount}>Create my Ehral account <i className="ti ti-arrow-right"/></button></div>}
