  ...
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(
        request.error ||
          new Error(
            "Could not open secure device storage.",
          ),
      );
  });
}
...
function keyForContext({
  ...
  /*
   * The cryptographic identity belongs to this
   * browser/device within an Ehral business,
   * not to an employee account.
   *
   * This is intentional.
   *
   * If employee A logs out and employee B signs
   * in on the same browser/device, Ehral must be
   * able to present the same device identity to
   * the backend so the backend can detect:
   *
   * MULTIPLE_EMPLOYEES_SAME_DEVICE
   *
   * The employee relationship is maintained
   * server-side by AttendanceDeviceBinding.
   */
  return businessId
    ? `business:${businessId}`
    : "default";
