...
export default function AttendanceSecurityPanel() {
  ...
  const openEvents = useMemo(
    () =>
      events.filter(
        (event) => event.status === "OPEN" || event.status === "UNDER_REVIEW",
      ),
    [events],
  );
  ...
  const resolve = async () => {
    ...
    setError("");
    try {
      const { data } = await resolveAttendanceSecurityEvent(
        selectedEvent.id,
        resolutionNote.trim(),
      );
      setEvents((current) =>
    ...
    <div className={styles.wrap}>
      <div className={styles.intro}>
        <div>
          ...
          <p>
            Manage registered attendance devices and review device-related
            attendance events for this business.
          </p>
        </div>
      ...
      <div className={styles.tabs}>
        <button
          className={tab === "devices" ? styles.active : ""}
        ...
        >
          Security events <span>{openEvents.length}</span>
        </button>
      </div>
                    ...
                    <tr key={`${device.employeeId}-${device.deviceId}`}>
                      ...
                      <td>
                        <button
                          ...
                          disabled={busyId === `revoke-${device.employeeId}`}
                          onClick={() => revoke(device)}
                        >
                          {busyId === `revoke-${device.employeeId}`
                            ? "Revoking…"
                            : "Revoke"}
                        </button>
                      </td>
        ...
        <div className={styles.card}>
          {events.length === 0 ? (
            <div className={styles.empty}>
              ...
              <strong>No attendance security events</strong>
              <span>
                Device-related events for this business will appear here when
            ...
            <div className={styles.eventList}>
              {events.map((event) => (
                ...
                >
                  ...
                  <div
                    className={`${styles.eventStatus} ${event.status === "RESOLVED" ? styles.resolved : ""}`}
                  >
                    {event.status}
                  </div>
        ...
        >
          ...
          >
            <div className={styles.modalHead}>
              <div>
                <span
                  className={`${styles.risk} ${selectedEvent.riskLevel === "MEDIUM" ? styles.medium : styles.high}`}
                >
                  {selectedEvent.riskLevel}
                </span>
                <h3>Attendance event</h3>
              </div>
            ...
            <div className={styles.detailGrid}>
              ...
              <div>
                <span>Time</span>
                <strong>{formatDate(selectedEvent.createdAt)}</strong>
              </div>
              <div>
                <span>Status</span>
              ...
              <div>
                <span>Device</span>
                <strong>{selectedEvent.deviceId || "No device proof"}</strong>
              </div>
            </div>
            <p className={styles.guidance}>
              Attendance was recorded and flagged for review. This event does
              ...
              selectedEvent.status !== "DISMISSED" && (
                <>
                  ...
                  <textarea
                    ...
                    placeholder="e.g. Employee replaced a damaged phone."
                    rows={3}
                  />
                  <div className={styles.modalActions}>
                    ...
                    <button
                      ...
                      disabled={busyId === `resolve-${selectedEvent.id}`}
                      onClick={resolve}
                    >
                      {busyId === `resolve-${selectedEvent.id}`
                        ? "Resolving…"
                        : "Resolve event"}
                    </button>
                  </div>
